...
public class HgncOwlWriter {
	...
	public HgncOwlWriter(HgncOntology hugoOntology, URI inSaveURI) {
		try {
			...
			// this.manager =
			// OntologyManagement.ontology.getOWLOntologyManager();
			ontologyIRI = IRI.create("http://ncicb.nci.nih.gov/genenames.org/HGNC.owl");
	...
	private void createHierarchyConcepts(HashMap<String, String> locusHierarchy) {
		...
		Vector<String> locusGroups = new Vector<>();
		for (String locusType : locusTypes) {
			...
			OWLClass locusTypeParent = factory.getOWLClass(createIRI(locusType));
			OWLClass locusGroupParent = factory.getOWLClass(createIRI(locusGroup));
			OWLAxiom axiom = factory.getOWLSubClassOfAxiom(locusTypeParent, locusGroupParent);
		...
		for (String locusGroup : locusGroups) {
			OWLClass locusGroupParent = factory.getOWLClass(createIRI(locusGroup));
			OWLAxiom axiom = factory.getOWLSubClassOfAxiom(locusGroupParent, factory.getOWLThing());
			AddAxiom addAxiom = new AddAxiom(ontology, axiom);
			try {
				manager.applyChange(addAxiom);
			} catch (Exception e) {
	...
	private void createConcepts(HgncOntology hugoOntology) {
		...
		for (HgncConcept concept : conceptVector) {
			...
			// + concept.code));
			OWLClass clz = factory.getOWLClass(createIRI(concept.code.replace(":", "_")));
			OWLClass parent = factory.getOWLClass(createIRI(concept.getParent()));
	...
	//
	//
	// OWLAnnotationProperty aProp =
	// this.manager.getOWLDataFactory().getOWLAnnotationProperty(createIRI(prop+"_ID"));
	...
	// OWLAnnotationProperty aProp =
	// this.manager.getOWLDataFactory().getOWLAnnotationProperty(createIRI(prop+"_LINK"));
	//// OWLEntityAnnotationAxiom ax1 = factory.getOWLEntityAnnotationAxiom(
	//// ent, anno);
	// OWLAnnotation anno = factory.getOWLAnnotation(
	// aProp, factory.getOWLLiteral(concept
	...
	private void addSpecializedProperties(HgncConcept concept, AddAxiom ax) {
		...
		OWLDatatype odt = factory.getOWLDatatype(IRI.create(typeConstantLiteral));
		...
		for (String prop : props) {
			...
			OWLAnnotation anno = factory.getOWLAnnotation(aProp, factory.getOWLLiteral(prop));
	...
	private void addLSDBhyperlinks(HgncConcept concept, AddAxiom ax) {
		...
		for (String prop : props) {
			...
			OWLAnnotation anno = factory.getOWLAnnotation(aProp, factory.getOWLLiteral(propURL));
			final OWLAxiom ax1 = factory.getOWLAnnotationAssertionAxiom(ent.getIRI(), anno);
			changes.add(new AddAxiom(ontology, ax1));
		}
	...
	private void addLRG_ID(HgncConcept concept, AddAxiom ax) {
		...
		for (String prop : props) {
			...
			OWLAnnotationProperty aProp = this.manager.getOWLDataFactory()
					.getOWLAnnotationProperty(createIRI("LRG_ID"));
			OWLAnnotation anno = factory.getOWLAnnotation(aProp, factory.getOWLLiteral(propID));
			// OWLEntityAnnotationAxiom ax1 =
			// factory.getOWLEntityAnnotationAxiom(
			// ent, anno);
			//
			final OWLAxiom ax1 = factory.getOWLAnnotationAssertionAxiom(ent.getIRI(), anno);
			...
			// OWLAnnotation anno = factory.getOWLConstantAnnotation(
			// createIRI(prop + "_LINK"), otc);
			OWLAnnotationProperty aProp_link = this.manager.getOWLDataFactory()
					.getOWLAnnotationProperty(createIRI("LRG"));
			// OWLEntityAnnotationAxiom ax1 =
			// factory.getOWLEntityAnnotationAxiom(
			// ent, anno);
			OWLAnnotation anno_link = factory.getOWLAnnotation(aProp_link, factory.getOWLLiteral(propLink));
			final OWLAxiom ax1_link = factory.getOWLAnnotationAssertionAxiom(ent.getIRI(), anno_link);
	...
	private void addlsdbComplexProperties(HgncConcept concept) {
		...
		for (SpecialistDatabase ldsbEntry : concept.getSpecialistDatabase()) {
			Set<OWLAnnotation> annoAnnotations = new HashSet<>();
	...
	private void addSimpleProperties(HashMap<String, String> properties, AddAxiom ax) {
	...
	private void addDelimitedProperties(HashMap<String, Vector<String>> properties, AddAxiom ax) {
		Set<OWLOntologyChange> changes = new HashSet<>();
		OWLEntity ent = null;
		OWLSubClassOfAxiom subAx = (OWLSubClassOfAxiom) ax.getAxiom();
		// for (OWLEntity e : ax.getEntities()) {
