...
/**
 * Writes the CTCAE Quick-Reference in RTF document format
 * based on input from the LexBIG system.
 */

public class QuickRefPublisherRTF {
	...
	public static final Color COLOR_COVER = Color.BLUE;
	
	// Input and output ...
	private Document document_ = null;
	private SchemeReader in_ = null;
	private RtfWriter2 rtfOut_ = null;
	...
	public static void main(String[] args) {
		...
		QuickRefPublisherRTF writer = null;
		try {
			reader = new SchemeReader();
			writer = new QuickRefPublisherRTF(reader);
			writer.publish();
		} finally {
			if (reader != null)
				reader.close();
	 ...
	 * Output will be written as RTF.
	 * @param reader
	 */
	public QuickRefPublisherRTF(SchemeReader reader) {
		super();
		in_ = reader;
		document_ = new Document();
		try {
			rtfOut_ = RtfWriter2.getInstance(document_,
					new FileOutputStream("CTCAEQuickReference.rtf"));
				rtfOut_.setAutogenerateTOCEntries(true);
			initDocument();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void close() {
		...
		if (rtfOut_ != null)
			try { rtfOut_.close(); } catch (Exception e) {}
	}
	 ...
	 * <p>
	 * For rtf, importing the pdf image results in error
	 * and import from the original source results in
	 * format issues.  The QuickRefIntroPage.png is
	 * imported as a placeholder.  If required, the rtf
	 * source can be directly inserted and formated as
	...
	protected void printIntro() throws Exception {
		...
		if (rtfOut_ != null) {
			...
			image.scaleAbsolute(
				document_.getPageSize().getWidth(),
				document_.getPageSize().getHeight()
					- 6f); // allow for margin
			rtfOut_.add(image);
			rtfOut_.flush();
		}
