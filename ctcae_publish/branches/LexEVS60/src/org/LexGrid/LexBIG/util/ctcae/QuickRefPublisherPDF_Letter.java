...
public class QuickRefPublisherPDF_Letter {
	...
	public static void main(String[] args) {
		...
		QuickRefPublisherPDF_Letter writer = null;
		try {
			reader = new SchemeReader();
			writer = new QuickRefPublisherPDF_Letter(reader);
			writer.publish();
	 ...
	 * @param reader
	 */
	public QuickRefPublisherPDF_Letter(SchemeReader reader) {
		super();
		in_ = reader;
		document_ = new Document();
		try {
			pdfOut_ = PdfWriter.getInstance(document_, new FileOutputStream(
			        "CTCAEQuickReference_8.5x11.pdf"));
	 ...
	 * QuickRefIntroPage.rtf (using a PDF writer such as PDF995) in Letter
	 * dimensions.
	 */
	protected void printIntro() throws Exception {
		try {
			...
			PdfReader reader = new PdfReader("QuickRefIntroPage_Letter.pdf");
	...
	protected SimpleCell getTextCell(String s, int style, int colspan,
		...
		case COLSTYLE_CATEGORY_HEADING:
			p = new Paragraph(s, FontFactory.getFont(BaseFont.HELVETICA_BOLD,
			        BaseFont.WINANSI, BaseFont.NOT_EMBEDDED, 7.0f));
